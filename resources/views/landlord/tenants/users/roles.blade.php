...
@section('title', 'User Roles — ' . trim($user->first_name . ' ' . $user->last_name))
                ...
                <form method="POST" action="{{ route('landlord.tenants.users.roles.update', [$tenant, $user]) }}" class="grid gap-7">
                    ...
                    <div class="mb-4 p-4 rounded-lg bg-warning/10 border border-warning/30">
                        <p class="text-sm text-warning">
                            ...
                            Changes made here bypass the tenant's own permission-escalation safeguard. Use this only to restore access the tenant admins are otherwise unable to grant themselves.
                        </p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                        @if($roles->isEmpty())
                            <div class="col-span-full">
                                <p class="text-sm text-muted-foreground">This tenant has no roles yet.</p>
                            </div>
                        @else
