    ...
    <div class="grid gap-5 lg:gap-7.5">
        <div class="fl-card">
            <div class="fl-card-header">
                <h3 class="fl-card-title">Tenant Details</h3>
            </div>
            <div class="fl-card-content">
                <form method="POST" action="{{ route('landlord.tenants.store') }}" class="grid gap-7">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        {{-- Tenant ID --}}
                        <div>
                            <label class="fl-form-label block mb-2" for="id">
                                Subdomain <span class="text-destructive">*</span>
                            </label>
                            <input id="id" name="id" type="text" value="{{ old('id') }}"
                                   class="fl-input w-full" placeholder="e.g. acme"
                        ...
                        <div>
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name') }}"
                                   class="fl-input w-full" placeholder="e.g. Acme Corp"
                        ...
                        {{-- Admin Password --}}
                        <div>
                            <label class="fl-form-label block mb-2" for="admin_password">
                                Admin Password <span class="text-destructive">*</span>
                            </label>
                            <input id="admin_password" name="admin_password" type="password"
                                   class="fl-input w-full" placeholder="Minimum 8 characters"
                        ...
                        <div class="lg:col-span-2">
                            <label class="fl-form-label block mb-2" for="idp_tenant_id">IDP Tenant</label>
                            @if(count($idpTenants) > 0)
                                <select id="idp_tenant_id" name="idp_tenant_id" class="fl-select w-full">
                            ...
                            @else
                                <input id="idp_tenant_id" name="idp_tenant_id" type="text" value="{{ old('idp_tenant_id') }}"
                                       class="fl-input w-full" placeholder="IDP tenant ID (optional)" />
                    ...
                    <div class="pt-5 mt-2 flex justify-start items-center gap-2.5">
                        ...
                        <a class="fl-btn fl-btn-light" href="{{ route('landlord.tenants.index') }}">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
