        ...
        <div class="sgh-card">
            ...
            <div class="sgh-card-content">
                <form id="update-role-form" method="POST" action="{{ route('roles.update', $role) }}" class="grid gap-7">
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        <div>
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name', $role->name) }}" class="sgh-input w-full" placeholder="e.g. Administrator" required aria-invalid="@error('name') true @else false @enderror" />
                ...
                <div class="pt-5 mt-2 flex justify-between items-center gap-2.5">
                    <div class="flex items-center gap-2.5">
                        <button type="submit" form="update-role-form" class="sgh-btn sgh-btn-primary">
                            <x-tabler-check-filled />
                            {{ __('roles.buttons.update') }}
                        </button>
                        <a class="sgh-btn sgh-btn-outline" href="{{ route('roles.permissions.edit', $role) }}">
                    ...
                    @can(\App\Enums\Tenant\PermissionKey::DeleteRole->value)
                        <button type="submit" form="delete-role-form" class="sgh-btn sgh-btn-danger">
