    ...
    <div class="flex flex-wrap items-center justify-between gap-5 pb-7.5 lg:items-end">
        ...
        <div class="flex items-center gap-2.5">
            ...
            @can(\App\Enums\Tenant\PermissionKey::CreateCashCount->value)
                <a class="sgh-btn sgh-btn-primary" href="{{ route('cash-count.create', $branch) }}">
    ...
    <div class="grid gap-5 lg:gap-7.5">
        <div class="sgh-card sgh-card-grid">
            <div class="sgh-card-header">
                <h3 class="sgh-card-title">{{ __('cash_count.history_title') }}</h3>
                                ...
                                <tr>
                                    <th class="min-w-[160px]">
                                        <span class="sgh-table-col">
                                            <span class="sgh-table-col-label">{{ __('cash_count.labels.counted_at') }}</span>
                                        </span>
                                    </th>
                                    <th class="min-w-[160px]">
                                        <span class="sgh-table-col">
                                            <span class="sgh-table-col-label">{{ __('cash_count.labels.counted_by') }}</span>
                                        </span>
                                    </th>
                                    <th class="min-w-[140px] text-right">
                                        <span class="sgh-table-col justify-end">
                                            <span class="sgh-table-col-label">{{ __('cash_count.labels.counted_total') }}</span>
                                        </span>
                                    </th>
                                    <th class="min-w-[140px] text-right">
                                        <span class="sgh-table-col justify-end">
                                            <span class="sgh-table-col-label">{{ __('cash_count.labels.balance_at_count') }}</span>
                                        </span>
                                    </th>
                                    <th class="min-w-[120px] text-center">
                                        <span class="sgh-table-col">
                                            <span class="sgh-table-col-label">{{ __('cash_count.labels.difference') }}</span>
                                        </span>
                                    </th>
                                ...
                                @foreach($cashCounts as $cashCount)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if($status === 'equal')
                                                <span class="badge badge-sm sgh-badge sgh-badge-success">{{ __('cash_count.status.equal') }}</span>
                                            @elseif($status === 'surplus')
                                                <span class="badge badge-sm sgh-badge sgh-badge-warning">
                                                    +{{ $cashbook->currency->symbol }} {{ number_format(abs((float) $cashCount->difference), 2) }}
                                                </span>
                                            @else
                                                <span class="badge badge-sm sgh-badge sgh-badge-danger">
                                                    −{{ $cashbook->currency->symbol }} {{ number_format(abs((float) $cashCount->difference), 2) }}
                                                </span>
                                            @endif
                                        </td>
