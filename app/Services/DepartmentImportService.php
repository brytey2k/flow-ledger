    ...
    public function import(UploadedFile $file): int
    {
        ...
        if ($path === false) {
            throw ValidationException::withMessages([
                'file' => __('departments.import_errors.unreadable'),
            ]);
        }
        ...
        try {
            ...
            if ($headers === false) {
                throw ValidationException::withMessages([
                    'file' => __('departments.import_errors.empty'),
                ]);
            }
            ...
            if ($headers !== ['name']) {
                throw ValidationException::withMessages([
                    'file' => __('departments.import_errors.invalid_headers'),
                ]);
            }
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                if ($name === '') {
                    $errors["rows.$rowNumber"] = __('departments.import_errors.name_required', ['row' => $rowNumber]);
                    continue;
                }

                if (mb_strlen($name) > 100) {
                    $errors["rows.$rowNumber"] = __('departments.import_errors.name_too_long', [
                        'row' => $rowNumber,
                        'name' => $name,
                    ]);
                    continue;
                ...
                if (isset($seenNames[$name])) {
                    $errors["rows.$rowNumber"] = __('departments.import_errors.duplicate_in_file', [
                        'row' => $rowNumber,
                        'name' => $name,
                    ]);
                    continue;
            ...
            if ($rows === []) {
                throw ValidationException::withMessages([
                    'file' => __('departments.import_errors.no_rows'),
                ]);
            }
            ...
            if ($existingNames !== []) {
                ...
                foreach ($rowNames as $rowNumber => $name) {
                    if (isset($existingLookup[$name])) {
                        $errors["rows.$rowNumber"] = __('departments.import_errors.duplicate_existing', [
                            'row' => $rowNumber,
                            'name' => $name,
                        ]);
                    }
